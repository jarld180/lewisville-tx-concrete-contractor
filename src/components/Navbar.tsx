...
const Navbar: React.FC = () => {
  ...
  const navLinks = [
    ...
    },
    {
      name: 'Locations',
      dropdown: [
        { name: 'Lewisville', path: '/' },
        { name: 'Flower Mound', path: '/flower-mound' },
        { name: 'Highland Village', path: '/highland-village' },
        ...
        <div className="p-8 h-full flex flex-col text-black">
          <div className="flex justify-between items-start mb-12 gap-4">
            <Link to={currentCity ? `/${currentCity}` : "/"} onClick={() => handleLinkClick(currentCity ? `/${currentCity}` : "/")} className="flex items-start space-x-2 min-w-0">
              ...
              <span className="text-xl font-bold uppercase tracking-tighter text-black truncate">Lewisville <span className="text-black">Concrete</span></span>
            </Link>
          ...
          <div className="mt-auto pt-8 border-t border-gray-100">
            ...
            <p className="text-center mt-4 text-black font-bold text-sm">(682) 255-2213</p>
          </div>
        </div>
      </div>
    </>
